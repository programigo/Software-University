...
public class Spy
{
    ...
    public string CollectGettersAndSetters(string className)
    {
        ...
        var allMethods = hackerType.GetMethods(BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Public);

        StringBuilder sb = new StringBuilder();

        foreach (var methodInfo in allMethods.Where(m => m.Name.Contains("get")))
        {
            sb.AppendLine($"{methodInfo.Name} will return {methodInfo.ReturnType.FullName}");
        }

        foreach (var methodInfo in allMethods.Where(m => m.Name.Contains("set")))
        {
